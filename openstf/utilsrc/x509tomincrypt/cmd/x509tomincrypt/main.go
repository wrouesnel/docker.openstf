...
func main() {
	...
	keyStruct := RSAPublicKey{}

	if privateKey.Size() != AndroidPubKeyModulusSize {
		fmt.Fprintln(os.Stderr, "ADB requires an RSA key of 2048 bits length exactly.")
		os.Exit(1)
	}

	// Store the modulus size
	keyStruct.Modulus_size_words = AndroidPubKeyModulusSizeWords
	...
	// Store the modulus
	copy(keyStruct.Modulus[:], reverseBytes(PaddedBigBytes(n,AndroidPubKeyModulusSize)))

	// Compute and store rr = (2^(rsa_size)) ^ 2 mod N.

	// Store rr
	rr := new(big.Int).SetBit(new(big.Int), AndroidPubKeyModulusSize * 8, 1)
	rr.Exp(rr,big.NewInt(2), n)
	copy(keyStruct.Rr[:], reverseBytes(PaddedBigBytes(rr,AndroidPubKeyModulusSize)))
